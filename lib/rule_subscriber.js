...
class BasicSubscription {
    constructor(options, services, rule) {
        ...
        this._executor = new RuleExecutor(rule, this._services, this._options);
        this._retryExecutor = new RetryExecutor(rule, this._services, this._options);
    }
...
class RegexTopicSubscription {
    constructor(options, services, rule, metadataWatch) {
        this._services = services;
        this._log = services.log;
        this._options = options;
        this._rule = rule;
    ...
    _subscribeTopic(topicName) {
        ...
        const executor = new RuleExecutor(topicRule, this._services, this._options);
        this._executors.push(executor);

        const retryExecutor = new RetryExecutor(topicRule, this._services, this._options);
...
class Subscriber {
    ...
    _createSubscription(rule) {
        if (!rule.isRegexRule()) {
            return P.resolve(new BasicSubscription(this._options, this._services,
                rule));
        }
        let maybeCreateWatchAction;
        if (!this._metadataWatch) {
            maybeCreateWatchAction =
                this._services.kafka.createMetadataWatch('metadata_refresher')
                .tap((refresher) => {
        ...
        .then(() => new RegexTopicSubscription(this._options, this._services,
            rule, this._metadataWatch));
    }

    subscribe(rule) {
        return this._createSubscription(rule)
        .then((subscription) => {
