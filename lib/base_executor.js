    ...
    _consume() {
        ...
        .then((messages) => {
            ...
            messages.forEach((msg) => {
                ...
                this._hyper.metrics.makeMetric({
                    type: 'Counter',
                    name: 'dequeue',
                    prometheus: {
                        name: 'changeprop_executor_dequeue_count_total',
                        help: 'base executor dequeue count'
                    },
                    sampleRate: 0.1,
                    labels: {
                        names: ['request_class', 'type'],
                        omitLabelNames: true
                    }
                }).increment(1, [this._hyper.requestClass, this.statName(message)]);
    ...
    _exec(origEvent, handler, statDelayStartTime, retryEvent) {
        ...
        if (this._isBlacklisted(origEvent)) {
            ...
            this._hyper.metrics.makeMetric({
                ...
                name: 'blacklist',
                prometheus: {
                    name: 'changeprop_executor_blacklist_count_total',
                    help: 'base executor blacklist count'
                },
                labels: {
                    names: ['request_class', 'type'],
                    omitLabelNames: true
                }
            }).increment(1, [this._hyper.requestClass, this.statName(origEvent)]);
        ...
        this._hyper.metrics.makeMetric({
            ...
            name: 'delay',
            prometheus: {
                name: 'changeprop_executor_delay_duration_seconds',
                help: 'base executor delay duration'
            },
            labels: {
                names: ['request_class', 'type'],
                omitLabelNames: true
            }
        }).endTiming(metricStartTime, [this._hyper.requestClass, this.statName(origEvent)]);
        ...
        if (origEvent.root_event && !retryEvent) {
            ...
            this._hyper.metrics.makeMetric({
                ...
                name: 'totaldelay',
                prometheus: {
                    name: 'changeprop_executor_totaldelay_duration_seconds',
                    help: 'base executor totaldelay duration'
                },
                labels: {
                    names: ['request_class', 'type'],
                    omitLabelNames: true
                ...
                [this._hyper.requestClass, this.statName(origEvent)]
            );
        }
        ...
        .then((isRateLimited) => {
            ...
            .then((messageDeduped) => {
                ...
                .finally(() => this._hyper.metrics.makeMetric({
                    type: 'Gauge',
                    name: 'exec',
                    prometheus: {
                        name: 'changeprop_executor_exec_duration_seconds',
                        help: 'base executor exec duration'
                    },
                    labels: {
                        names: ['request_class', 'type'],
                        omitLabelNames: true
                    }
                }).endTiming(startTime, [this._hyper.requestClass, this.statName(origEvent)]));
            });
        });
    }
