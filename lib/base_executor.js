...
class BaseExecutor {
        ...
        .then((consumer) => {
            ...
            this.consumer.on('error', (e) => {
                if (e.code !== kafka.CODES.ERRORS.ERR__PARTITION_EOF
                        && e.code !== kafka.CODES.ERRORS.ERR__TIMED_OUT) {
    ...
    _consume() {
        this.consumer.consumeAsync(this.concurrency)
        .catch((e) => {
            if (e.code === kafka.CODES.ERRORS.ERR__PARTITION_EOF) {
                return P.delay(100);
            }
            // Ignore errors here, they will be handled by the generic consumer error handler
        })
        .then((messages) => {
            ...
            return P.all(messages.map((msg) => {
                this._pendingMsgs.push(msg);
                return this._safeParse(msg.message.toString('utf8'))
        ...
        .finally(() => this._consume());
    }

    _notifyFinished(finishedMsg) {
        if (this._pendingMsgs) {
    ...
    _doCommit() {
        if (this._toCommit) {
            this._commitTimeout = null;
            return this.consumer.commitAsync(this._toCommit)
            .then(() => {
