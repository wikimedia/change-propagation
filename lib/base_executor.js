...
class BaseExecutor {
    ...
    constructor(rule, kafkaFactory, hyper, log, options) {
        ...
        if (options.templates.hosts) {
            this.hostsTemplate = new Template(options.templates.hosts);
        }
    ...
    _exec(origEvent, optionIndex, statName, statDelayStartTime, retryEvent) {
        ...
        if (this.hostsTemplate) {
            context.hosts = this.hostsTemplate.expand(context);
        }
        return P.each(rule.getExec(optionIndex), (tpl) => {
            const request = tpl.expand(context);
