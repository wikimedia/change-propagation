...
class BaseExecutor {
     ...
     */
    _reportSuccess(expander, evCtx) {
        const timeDelta = Date.now() - evCtx.startTime;
        return P.all([
            this._updateLimiters(expander, 200, timeDelta),
            this.budgeteer.reportSuccess(evCtx.budgetKey,
                this.budget, evCtx.startTime, timeDelta / 1000)
        ]);
    }
     ...
     * Temporary internal wrapper around limiter & budgeteer.
     */
    _reportFailure(error, expander, evCtx) {
        const timeDelta = Date.now() - evCtx.startTime;
        return P.all([
            this._updateLimiters(expander, error.status, timeDelta),
            this.budgeteer.reportScheduled(evCtx.budgetKey, this.budget, evCtx.startTime,
                timeDelta / 1000)
        ]);
    }
    ...
    _dedupeMessage(expander, evCtx) {
        ...
        .then((checkResult) => {
            ...
            return this.budgeteer.check(evCtx.rootEventBudgetKey, this.budget, evCtx.eventTime)
            .then((rootCheckRes) => {
    ...
    _exec(origEvent, handler, retryEvent) {
        ...
        this.hyper.metrics.endTiming([`${this.statName}_delay`],
            retryEvent && new Date(retryEvent.meta.dt) || evCtx.eventTime);
        ...
        return this._rateLimitMessage(expander, evCtx)
        .then((isRateLimited) => {
            if (isRateLimited) {
                return { status: 200 };
            }
            return this._dedupeMessage(expander, evCtx)
            .then((messageDeduped) => {
                if (messageDeduped) {
                    return { status: 200 };
                }
                ...
                .tap(() => this._reportSuccess(expander, evCtx))
                .tapCatch(err => this._reportFailure(err, expander, evCtx))
