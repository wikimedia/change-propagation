...
class BaseExecutor {
    ...
    _dedupeMessage(expander, evCtx) {
        ...
        .then((checkResult) => {
            ...
            const rootEventKey = `${this._prefix}_dedupe_${name}`
                + `_${message.root_event.signature}`;
            ...
            return this.budgeteer.check(rootEventKey, this.budget, evCtx.eventTime)
            .then((rootCheckRes) => {
    ...
    _exec(origEvent, handler, retryEvent) {
        ...
        const eventKey = `${rule.name}_${origEvent.sha1 || origEvent.meta.id}`;
