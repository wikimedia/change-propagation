...
class RuleExecutor {
    ...
    _retryTopicName() {
        return `${this.kafkaFactory.consumeDC}.change-prop.retry.${this.rule.topic}`;
    }
    ...
    consumeRetry() {
        ...
        .then((msg) => {
            ...
            return this._safeParse(msg.payload.toString())
            .then((message) => {
                if (!message) {
                ...
                return this.taskQueue.enqueue(new Task(this.retryConsumer,
                    `change-prop-${this._retryTopicName()}-${this.rule.name}`,
                    message,
     ...
     */
    _setUpRetryTopic() {
        this.retryConsumer = new kafka.KafkaConsumer({
            "default_topic_conf": {
                "auto.offset.reset": "largest"
            },
            "group.id": `change-prop-${this._retryTopicName()}-${this.rule.name}`,
            // TODO: configure
        ...
        this.retryConsumer.subscribe([this._retryTopicName()]);
        this.taskQueue.addConsumer(this.retryConsumer);
        this.consumeRetry();
    }
    ...
    consume() {
        ...
        .then((msg) => {
            ...
            .then((message) => {
                ...
                console.log('OPTION for rule ' + this.rule.name + ' ' + optionIndex);
                if (optionIndex === -1) {
                    // Message doesn't match any option for this rule, we're done
                    return this.consume();
                }

                console.log('PUSH TO THE QUEUE', this.rule.name);
    ...
    subscribe() {
        const rule = this.rule;
        this._setUpRetryTopic();
        this.consumer = new kafka.KafkaConsumer({
            "default_topic_conf": {
                "auto.offset.reset": "largest"
            },
            "group.id": `change-prop-${rule.name}`,
            // TODO: configure
            "metadata.broker.list": "127.0.0.1:9092",
            "fetch.wait.max.ms": "1",
            "fetch.min.bytes": "1",
        ...
        console.log(`${this.kafkaFactory.consumeDC}.${rule.topic}`);
        this.consumer.subscribe([ `${this.kafkaFactory.consumeDC}.${rule.topic}` ]);
        this.taskQueue.addConsumer(this.consumer);
        // TODO: this is actually making it run one-by-one.
        this.consume();
        // TODO
        return P.resolve()
    /* TODO        .catch(TopicsNotExistError, (e) => {
    ...
    close() {
   // TODO:     return this.taskQueue.close()
